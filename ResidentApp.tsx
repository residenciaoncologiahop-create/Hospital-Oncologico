...
// --- TIPOS LOCALES (Aislados del modo normal) ---
interface ResidentPatient {
  ...
  timeline: any[]; // Se definirá mejor al integrar los módulos
  chatHistory: { role: 'user' | 'model'; text: string; timestamp: number }[];
  lastUpdated: number;
}

const ResidentApp = () => {
  // --- ESTADO VOLÁTIL (Solo en memoria) ---
  ...
  // --- HANDLERS (Lógica Local) ---

  const handleCreatePatient = (e: React.FormEvent) => {
    e.preventDefault();
    const newPatient: ResidentPatient = {
      id: `res-${Date.now()}`, // ID temporal
      name: newName,
  ...
  const handleDeletePatient = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (window.confirm("¿Descartar este caso temporal? Se perderán los datos.")) {
  ...
  const handleExit = () => {
    if (window.confirm("Al salir se borrarán todos los casos cargados en esta sesión. ¿Confirmar?")) {
      window.location.reload(); // Forma más segura de limpiar la memoria volátil
    }
  };
  ...
  return (
    <div className="flex h-screen bg-white text-gray-800 font-sans text-xs overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 bg-gray-50 border-r transform lg:translate-x-0 lg:static flex flex-col transition-transform duration-300 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <div className="px-2 mb-3">
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-8 pr-3 py-2 bg-white border border-gray-200 rounded-lg text-[11px] outline-none focus:border-indigo-300 transition-all"
                value={searchTerm}
          ...
          <div className="space-y-1.5">
            {filteredPatients.length === 0 && (
              <div className="text-center py-8 opacity-40">
                <p className="text-[10px] font-bold text-gray-400">Sin casos activos</p>
              </div>
            )}
            {filteredPatients.map(p => (
              <div 
                ...
                className={`group w-full text-left p-3 rounded-xl transition-all flex items-center justify-between cursor-pointer ${selectedId === p.id ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'hover:bg-white border border-transparent hover:border-gray-100'}`}
              >
                ...
                <button onClick={(e) => handleDeletePatient(p.id, e)} className={`p-1.5 rounded-full hover:bg-red-100 hover:text-red-500 transition-colors ${selectedId === p.id ? 'text-indigo-300 hover:text-white hover:bg-indigo-500' : 'text-gray-300 opacity-0 group-hover:opacity-100'}`}>
                  <Trash2 size={12} />
                </button>
              </div>
            ))}
        ...
        <div className="p-4 border-t bg-white">
          <button onClick={handleExit} className="w-full flex items-center justify-center space-x-2 text-gray-400 hover:text-red-500 transition-colors p-2 rounded-lg hover:bg-red-50">
            <LogOut size={14} />
            <span className="text-[10px] font-bold uppercase tracking-widest">Salir del Modo Residente</span>
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden bg-gray-50">
          ...
          <div className="flex items-center space-x-4">
            ...
            {selectedPatient && (
              <button 
                ...
                className="hidden lg:block text-gray-400 hover:text-indigo-600 transition-colors"
              >
            ...
            <div className="flex flex-col">
              <h1 className="font-black text-gray-800 text-lg tracking-tight leading-none truncate max-w-md">
                {selectedPatient ? selectedPatient.name : 'Bienvenido, Residente'}
              </h1>
        ...
        {selectedPatient ? (
          <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
            ...
            {/* Panel de Trabajo (Documentación y Herramientas) */}
            <div className={`${showLeftPanel ? 'lg:w-1/2 border-r' : 'hidden'} flex flex-col bg-white h-full transition-all duration-300`}>
              ...
              <div className="flex-1 overflow-y-auto p-8 scrollbar-hide">
                {activeTab === 'docs' && (
                  <div className="space-y-6">
                    <div className="p-4 bg-yellow-50 border border-yellow-100 rounded-xl text-yellow-800 text-xs">
                      ...
                      <p>Recuerde que los archivos y notas que cargue aquí desaparecerán al cerrar la pestaña.</p>
                    </div>
                    {/* Placeholder para FileUploader y Textarea (Se implementarán/reutilizarán en el paso de integración final) */}
                    <div className="border-2 border-dashed border-gray-200 rounded-xl h-32 flex items-center justify-center text-gray-400 text-xs font-bold uppercase">
                ...
                {activeTab === 'timeline' && (
                   <div className="text-center py-10 text-gray-400 font-bold text-xs uppercase">[Línea de Tiempo del Caso]</div>
                )}

                {activeTab === 'forms' && (
                   // Reutilizamos el componente FormManager existente pasándole los datos locales
                   <FormManager 
                     patient={selectedPatient} 
                     historyText={selectedPatient.historyText} 
            ...
            <div className={`${showLeftPanel ? 'lg:w-1/2' : 'w-full'} flex flex-col bg-gray-50 h-full overflow-hidden`}>
               <div className="flex-1 flex items-center justify-center text-gray-300 font-bold text-xs uppercase tracking-widest">
                 [Chat Clínico de Residente]
               </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center opacity-80">
            <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-gray-100 max-w-sm">
              ...
              <button onClick={() => setShowNewPatientModal(true)} className="mt-8 bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black text-xs tracking-widest hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100 uppercase">
                Iniciar Caso
              </button>
            </div>
          </div>
      ...
      {/* --- MODAL NUEVO PACIENTE (RESIDENTE) --- */}
      {showNewPatientModal && (
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-300">
            ...
            <form onSubmit={handleCreatePatient} className="p-8 space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-1">Identificación (Iniciales/Ficticio)</label>
                <input autoFocus type="text" required className="w-full px-5 py-3 bg-gray-50 border-2 border-transparent rounded-xl text-sm font-bold focus:bg-white focus:border-indigo-200 outline-none transition-all" placeholder="Ej: Paciente A.B." value={newName} onChange={e => setNewName(e.target.value)} />
              </div>
              <div className="flex space-x-4">
                <div className="w-1/3 space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-1">Edad</label>
                  <input type="number" required className="w-full px-5 py-3 bg-gray-50 border-2 border-transparent rounded-xl text-sm font-bold focus:bg-white focus:border-indigo-200 outline-none transition-all" placeholder="--" value={newAge} onChange={e => setNewAge(e.target.value)} />
                </div>
                <div className="w-2/3 space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-1">Diagnóstico Base</label>
                  <input type="text" required className="w-full px-5 py-3 bg-gray-50 border-2 border-transparent rounded-xl text-sm font-bold focus:bg-white focus:border-indigo-200 outline-none transition-all" placeholder="Ej: Ca Pulmón" value={newDx} onChange={e => setNewDx(e.target.value)} />
                </div>
              </div>
              <button type="submit" className="w-full bg-indigo-600 text-white py-4 rounded-xl text-xs font-black shadow-xl shadow-indigo-200 hover:bg-indigo-700 transition-all uppercase tracking-widest">
                Crear Caso Temporal
              </button>
            </form>
          </div>
