...
const App = () => {
    ...
    const savePatientDetails = async () => {
        if (selectedPatientId) {
            ...
            logAction("UPDATE_PATIENT_DATA", selectedPatientId, doctorName); // LOG AUDITORÍA
        }
    };
    ...
    const handleProcessDocuments = async () => {
        ...
        try {
            ...
            if (selectedPatientId) {
                ...
                logAction("PROCESS_DOCUMENTS", selectedPatientId, doctorName); // LOG AUDITORÍA
            }
    ...
    const handleAddManualEvolution = async () => {
        ...
        logAction("ADD_MANUAL_EVOLUTION", selectedPatientId, doctorName); // LOG AUDITORÍA
    };
    ...
    const handleDeleteEvent = async (indexToDelete: number) => {
        ...
        if (confirm("¿Eliminar este evento?")) {
            ...
            const patientRef = doc(db, "patients", selectedPatientId);
            await updateDoc(patientRef, { timeline: updatedTimeline, lastUpdated: Date.now() });
            logAction("DELETE_TIMELINE_EVENT", selectedPatientId, doctorName); // LOG AUDITORÍA
        }
    };
    ...
    const handleGenerateSummary = async () => {
        ...
        logAction("GENERATE_SUMMARY", selectedPatientId, doctorName); // LOG AUDITORÍA
    };
    ...
    const handleGenerateFollowUp = async () => {
        ...
        logAction("GENERATE_FOLLOWUP", selectedPatientId, doctorName); // LOG AUDITORÍA
    };
    ...
    const handleGenerateTumorBoard = async () => {
        ...
        logAction("GENERATE_TUMOR_BOARD", selectedPatientId, doctorName); // LOG AUDITORÍA
    };
    ...
    const handleCreatePatient = async (e: React.FormEvent) => {
        ...
        try {
            ...
            logAction("CREATE_PATIENT", docRef.id, doctorName); // LOG AUDITORÍA
    ...
    const handleDeletePatient = async (id: string, e: React.MouseEvent) => {
        ...
        if (confirm("¿Eliminar paciente permanentemente?")) {
            try {
                ...
                logAction("DELETE_PATIENT", id, doctorName); // LOG AUDITORÍA
