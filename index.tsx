...
const extractTimelineFromDocs = async (
    ...
    // NOTA: Usamos VITE_API_KEY (la de Gemini)
    const apiKey = import.meta.env.VITE_API_KEY;
    if (!apiKey) throw new Error("API_KEY_MISSING");

    try {
        const ai = new GoogleGenAI({ apiKey });
        // REQUISITO: Usar gemini-2.5-flash
...
const App = () => {
    ...
    // --- FIREBASE: Cargar Pacientes ---
    useEffect(() => {
        // Suscripción en tiempo real a la colección "patients"
    ...
    const handleProcessDocuments = async () => {
        ...
        try {
            ...
            // FIREBASE: Actualizar Timeline y Texto
            if (selectedPatientId) {
    ...
    const handleSendMessage = async () => {
        ...
        // FIREBASE: Actualizar Chat History
        const patientRef = doc(db, "patients", selectedPatientId);
        await updateDoc(patientRef, {
    ...
    const handleCreatePatient = async (e: React.FormEvent) => {
        ...
        try {
            // FIREBASE: Crear Paciente
            const docRef = await addDoc(collection(db, "patients"), p);
            
            // Firebase nos devuelve el ID, seleccionamos el paciente
