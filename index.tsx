...
// --- AI FUNCTIONS (CON PROTECCIÓN DE PRIVACIDAD ACTIVADA) ---
...
const extractTimelineFromDocs = async (text: string, files: FileData[]): Promise<ClinicalEvent[]> => {
    ...
    try {
        ...
        const parts: any[] = [{ text: `
            ...
            - NO incluyas DNI, CUIT, direcciones exactas ni teléfonos en la salida.
...
const generateText = async (prompt: string, context: string, files: FileData[]) => {
    ...
    // REGLA DE PRIVACIDAD INYECTADA
    const privacyRule = "\n\nIMPORTANTE: Protege la privacidad del paciente. NO incluyas números de DNI, pasaporte, teléfonos ni direcciones de domicilio en el texto generado.";
...
const getChatResponse = async (msgs: ChatMessage[], newMsg: string, context: string, files: FileData[]) => {
    ...
    const res = await ai.models.generateContent({
        model: 'gemini-2.5-flash',
        contents: { parts },
        config: { systemInstruction: "Eres un oncólogo experto. Responde en español técnico. NUNCA menciones DNI o datos de contacto personales." }
    });
    return res.text || "Error.";
};
...
const App = () => {
    ...
    useEffect(() => {
        ...
        getOrInitFingerprint(); // INIT FINGERPRINT
    }, []);
    ...
    useEffect(() => {
        if (selectedPatientId) {
            ...
            if (p) {
                ...
                setShowLeftPanel(true); // Reset view
            }
        }
    }, [selectedPatientId]);
