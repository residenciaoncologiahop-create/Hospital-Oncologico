...
const FileUploader: React.FC<FileUploaderProps> = ({ label, files, setFiles, accept = "application/pdf,image/*" }) => {
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        {files.map((f, i) => (
          <div key={i} className="flex items-center bg-indigo-50 text-indigo-700 px-2.5 py-1 rounded-lg text-[10px] border border-indigo-100 font-bold">
            ...
            <button onClick={() => setFiles(files.filter((_, idx) => idx !== i))} className="ml-1 text-indigo-300 hover:text-indigo-600"><X size={12} /></button>
          </div>
        ))}
      </div>
